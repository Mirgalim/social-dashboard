...
export async function cseSearch(q, max = 10) {
  ...
  return memo(`cse:${q}:${max}`, 60_000, async () => {
    ...
    const qs = new URLSearchParams({ key: KEY, cx: CX, q: `${q} ${filter}`, num: String(Math.min(max, 10)) });
    const j = await fetchJSON(`https://www.googleapis.com/customsearch/v1?${qs}`);
    return (j.items || []).map(it => {
      const mt = it.pagemap?.metatags?.[0] || {};
      const dt = mt["article:published_time"] || mt["og:updated_time"] || mt["og:published_time"] || null;
      const u = new URL(it.link);
      return { platform: hostToPlatform(u.hostname), text: T(it.title || it.snippet), date: dt, url: it.link,
